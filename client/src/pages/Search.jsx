...
const Search = () => {
  ...
  const filteredShowtimes = showtimes
    .filter((showtime) => {
      ...
      let pastDateCheck = true;
      if (filterPastDate) {
        const pastDate = new Date();
        pastDate.setDate(pastDate.getDate() - filterPastDate);
        pastDateCheck = showtimeDate >= pastDate && showtimeDate < new Date();
      }
  ...
  return (
    <div className="flex min-h-screen flex-col gap-4 bg-gray-900 pb-8 text-gray-900 sm:gap-8" >
      <Navbar />
      <div className="mx-4 flex h-fit flex-col gap-2 rounded-lg bg-slate-400 p-4 drop-shadow-xl sm:mx-8 sm:p-6">
        <h2 className="text-3xl font-bold text-gray-900">Search Showtimes</h2>
        <div className="flex flex-col gap-2 rounded-md bg-gradient-to-br from-indigo-200 to-indigo-200 p-4 transition-all duration-500 ease-in-out">
          <div
                    ...
                    <Command>
                      ...
                      <CommandGroup>
                        {showtimes.map((showtime) => (
                          <CommandItem
                            key={showtime.theater.cinema._id}
                            value={showtime.theater.cinema.name}
                ...
                <div className="flex flex-col items-start gap-x-8 gap-y-2 sm:flex-row sm:items-center">
                  <label className="text-md flex items-center justify-between gap-2 font-semibold text-gray-800">
                    ...
                    {[30, 60, 90].map((days) => (
                      <button
                        key={days}
                        onClick={() => setFilterPastDate(filterPastDate === days ? null : days)}
                        className={`px-4 py-2 text-white rounded-md ${filterPastDate === days ? 'bg-blue-600' : 'bg-blue-400'}`}
                      >
                        Last {days} Days
                      </button>
                        ))}
        ...
        <div
          className={`mb-4 grid max-h-screen overflow-auto rounded-md bg-gradient-to-br from-indigo-200 to-indigo-200`}
          style={{
            gridTemplateColumns:
